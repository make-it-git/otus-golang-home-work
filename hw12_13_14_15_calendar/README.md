...
Для запуска всех сервисов одной командой выполнить `make -j4 run`. Это запустит требуемые зависимости в docker (postgresql, rabbitmq), а также соберет и запустит с корректными конфигами все три сервиса. Важно указать именно `-j4`, иначе make не выполнит 4 задачи параллельно.